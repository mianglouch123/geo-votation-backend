...
// backend/src/services/mail.service.js (añade estos métodos)
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          ...
          <p style="font-size: 11px;">Si no reconoces esta actividad, por favor ignora este mensaje.</p>
        </div>
      </div>
    </body>
    </html>
...
async sendAnswerSummaryEmail(to, userName, votationTitle, votationDescription, questions) {
  try {
    ...
    console.log("📨 Email de resumen de respuestas enviado:", info.messageId);
    return info;
  } catch (err) {
    ...
    // No lanzamos el error para no bloquear la respuesta principal
    return null;
  }
}
...
async sendAnswerUpdateSummaryEmail(to, userName, votationTitle, votationDescription, questions) {
  try {
    ...
    console.log("📨 Email de actualización de respuestas enviado:", info.messageId);
    return info;
  } catch (err) {
